...
def get_hatnet_lc_locations(hatid, photometry_type='AP', project_id=0, duplicate=False, redlc=True, epdlc=True, tfalc=True, database=None):
    '''
...
def get_hatsouth_lc_locations(hatid, rawlc=True, epdlc=True,tfalc=True,focusframes=False,database=None):
    '''
...
def scp_fetch_hatsouth_lcs(remote_files, local_files,field_names,field_nums, hs_lcrepo='/S/LC',user=None,randomizehost=False, useinternalnet=True):
    '''
...
def scp_fetch_hatnet_lcs(lc_locations,user=None,destination=None,useinternalnet=True):
    '''
    This uses scp to fetch the probable light-curves from the remote servers.
...
def collect_fetched_lightcurves(hatid, lcdir=None, outdir=None,removefetched=False,saveinfo=False, ignorecollected=True):
    '''
        ...
        for ext in extensions:
            ...
            if len(extlcs) > 0 and ext == 'rawlc' and net == 'hs':
                ...
                for lcind, lc in enumerate(extlcs):
                    ...
                    rawlc = hatbinlc.read_lc(lc, 100, False)
                    if lcind == 0:
...
def consolidate_hatnet_lightcurves(collected_lc_dict, removecollected=False):
    '''
...
def consolidate_all_lightcurves(collected_lc_dict, removecollected=False):
    '''
...
def process_consolidated_lightcurve(hatid, datatable, requestedcols, database=None):
    '''
