...
def __spread__(np.float64_t y,
              np.ndarray[np.complex128_t,ndim=1] yy,
              np.int64_t n,
              np.float64_t x,
              np.int64_t m ):
  """
  ...
  """
  cdef:
    np.ndarray[np.int64_t,ndim=1]   nfac
    np.int64_t                      ix, j, ilo, ihi, nden 
    np.float64_t                    fac
  
  nfac = np.array([0,1,1,2,6,24,120,720,5040,40320,362880])
...
def fasper( np.ndarray[np.float64_t,ndim=1] x, 
  ...
  """
  cdef: 
    np.int64_t                          n, nout, nfreqt, nfreq, ndim, fndim, jmax
    np.float64_t                        ave, var, xmin, xmax, xdif, fac, df, pmax
    np.ndarray[np.float64_t, ndim=1]    wk1_new, wk2_new, ck, ckk, rwk1, iwk1, rwk2, iwk2, 
    np.ndarray[np.float64_t, ndim=1]    hypo2, hc2wt, hs2wt, cwt, swt, den, cterm, sterm
    np.ndarray[np.complex128_t, ndim=1] wk1, wk2
  ...
  nout  = int(0.5*float(ofac*hifac*n))
  ...
  wk1_new = df*(np.arange(nout, dtype='float')+1.)
  wk2_new = (cterm+sterm)/(2.0*var)
  pmax = wk2_new.max()
  jmax = wk2_new.argmax()
  ...
  return wk1_new,wk2_new,nout,jmax,prob
