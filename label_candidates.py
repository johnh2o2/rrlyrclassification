...
candidate_filenames = [ settings.get_gzipped_csv_lc_fname(ID) for ID in candidate_ids ]
...
app = Visualizer(root, candidate_filenames, 
					logfile=candidate_results_fname, flag_shortcuts=shortcuts, flags=flags)
root.mainloop()


# Now read in and interpret the results
logprint(" label_candidates : Now reading in the results")
...
f.close()
logprint(" label_candidates : done.")

# Now save these into the global "labeled HatIDs" file.
logprint(" label_candidates : Now saving the results!")
